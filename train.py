...
from lr_scheduler.warmuppoly import Warmup_PolyDecay
...
def parse_args():
    ...
    '--warmup',
    help=
    'use warmup for first 1000 iter. use Momentum',
    type=bool,
    default=True)
    parser.add_argument(
    '--warmup_steps',
...
def main(args):
    ...
    if args.warmup:
        lr = Warmup_PolyDecay(lr_rate=args.learning_rate,warmup_steps=args.warmup_steps,iters=cfg.iters,end_lr=1e-5)
        optimizer = paddle.optimizer.Momentum(lr, parameters=cfg.model.parameters(), momentum=0.9, weight_decay=5.0e-4)
    else:
